...
BEGIN
    ...
    INSERT INTO publication_master (publication_id, type, detail1, detail2, detail3, detail4)
    SELECT publication_id, 'publication', detail1, detail2, detail3, detail4
    FROM publication;
    total_records := total_records + SQL%ROWCOUNT;

    -- Merge data from the proceedings table
    INSERT INTO publication_master (publication_id, type, detail1, detail2, detail3, detail4)
    SELECT publication_id, 'proceedings', detail1, detail2, detail3, detail4
    FROM proceedings;
    total_records := total_records + SQL%ROWCOUNT;

    -- Merge data from the journal table
    INSERT INTO publication_master (publication_id, type, detail1, detail2, detail3, detail4)
    SELECT publication_id, 'journal', detail1, detail2, detail3, detail4
    FROM journal;
    total_records := total_records + SQL%ROWCOUNT;

    -- Merge data from the book table
    INSERT INTO publication_master (publication_id, type, detail1, detail2, detail3, detail4)
    SELECT publication_id, 'book', detail1, detail2, detail3, detail4
    FROM book;
    total_records := total_records + SQL%ROWCOUNT;

    -- Merge data from the article table
    INSERT INTO publication_master (publication_id, type, detail1, detail2, detail3, detail4)
    SELECT publication_id, 'article', detail1, detail2, detail3, detail4
    FROM article;
    total_records := total_records + SQL%ROWCOUNT;
